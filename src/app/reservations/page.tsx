...
export default function ReservationPage() {
  ...
  useEffect(() => {
    if (cabinId) {
      ...
      if (cabin) {
        ...
        const cabinBookedDates = getBookedDates(cabinId).map(dateStr => new Date(dateStr + 'T00:00:00')); // Ensure correct date object parsing
  ...
  const handleConfirmReservation = async () => {
    ...
    const reservationData: Omit<Reservation, 'id' | 'createdAt'> = {
      ...
      userId: 'testUser123', // Placeholder for actual user ID
      checkInDate: format(checkInDate, 'yyyy-MM-dd'),
      checkOutDate: format(addDays(checkInDate, 1), 'yyyy-MM-dd'), // Assuming 1-night stay
    ...
    try {
      ...
      // For now, using the in-memory store directly
